...
def build_translation_prompt(texts: list[str], target_lang: str, source_lang: str) -> str:
    ...
    return (
        ...
        f"3. Keep UNCHANGED: numbers, currencies (₹ £ $ €), percentages, dates, "
        f"and universally-recognised financial/technical abbreviations "
        f"(e.g. EBITDA, FY26E, QoQ, YoY, BEV, SoTP, CAGR, NM, CMP, JLR, TML, "
        f"CV, PV, AI, SMS, email addresses, URLs).\n"
        f"3b. For brand or product names written in Latin script (e.g. TGS, Screener, "
        f"Riise, WhatsApp): if {target_lang} uses a non-Latin script (Devanagari, "
        f"Tamil, Telugu, Kannada, Malayalam, Gujarati, Gurmukhi, Bengali, etc.), "
        f"write them PHONETICALLY in the {target_lang} script — do NOT keep Latin "
        f"letters mixed into otherwise non-Latin text "
        f"(e.g. 'TGS' → 'टीजीएस' in Hindi, 'Screener' → 'स्क्रीनर').\n"
